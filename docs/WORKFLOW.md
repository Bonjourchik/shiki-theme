...
3. Сайт хранит **собранный** CSS и пересобирает его только при сохранении поля — `git push` сам по себе ничего
   не меняет. Надёжно: в поле «Внешний вид сайта» увеличить `?v=` в адресе `@import` (новый адрес не в кеше)
   и нажать «Сохранить». Или: `https://shikimori.io/tests/reset_styles_cache` с URL из `config/field.css`,
   затем то же «Сохранить» без изменений. Проверка: на любой странице в `#custom_css` есть свежие правила.
